...
namespace Tobii.XR
{
    public partial class TobiiXRAdvanced
    {
        private readonly TobiiProvider _provider;
        private GCHandle _thisPointer;
        private int InternalQueueCapacity => TobiiProvider.AdvancedDataQueueSize;
        
        public TobiiXRAdvanced(TobiiProvider provider)
        {
            _provider = provider;
            _thisPointer = GCHandle.Alloc(this);
        }
        
        ~TobiiXRAdvanced()
        {
            if (_thisPointer.IsAllocated) _thisPointer.Free();
        }
