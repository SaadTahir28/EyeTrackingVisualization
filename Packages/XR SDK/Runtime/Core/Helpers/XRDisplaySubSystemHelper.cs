...
public static class XRDisplaySubSystemHelper
{
    ...
    public static bool TryGetRenderPasses(out XRDisplaySubsystem.XRRenderPass left, out XRDisplaySubsystem.XRRenderPass right)
    {
        left = default;
        right = default;
        ...
        for (var renderPassIndex = 0; renderPassIndex < renderPassCount; renderPassIndex++)
        {
            ...
            if (renderPass.renderTargetDesc.vrUsage == VRTextureUsage.TwoEyes)
            {
                left = right = renderPass;
                return true;
            }
