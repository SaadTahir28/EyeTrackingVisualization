...
namespace Tobii.XR
{
    ...
    {
        public static IControllerAdapter ControllerAdapter = new DummyControllerAdapter();
        ...
        private static void SetAdapter()
        {
            ...
            {
                ControllerAdapter = new OpenVRControllerAdapter();
            }
            else
            {
                ControllerAdapter = new XRInputSystemControllerAdapter();
            }
#else
            ControllerAdapter = new XRInputSystemControllerAdapter();
#endif
        ...
        private void Awake()
        {
            ...
            if (ControllerAdapter == null) SetAdapter();
        ...
        {
            VelocityUpdate();
            if (ControllerAdapter.GetType() == typeof(DummyControllerAdapter)) SetAdapter();
        ...
        {
            // Velocity
            var velocity = (ControllerAdapter.Position - _lastPosition) / Time.deltaTime;
            _velocity = VelocitySmoothingAlpha * velocity + (1 - VelocitySmoothingAlpha) * _velocity;

            // Angular rotation
            var velocityDiff = (ControllerAdapter.Rotation * Quaternion.Inverse(_lastRotation));
            ...
            _lastPosition = ControllerAdapter.Position;
            _lastRotation = ControllerAdapter.Rotation;
        }
        ...
        public Vector3 Position => _roomOrigin.TransformPoint(ControllerAdapter.Position);

        public Quaternion Rotation => _roomOrigin.rotation * ControllerAdapter.Rotation;
        ...
        public bool GetButtonPress(ControllerButton button)
        {
            return ControllerAdapter.GetButtonPress(button);
        }

        public bool GetButtonPressDown(ControllerButton button)
        {
            return ControllerAdapter.GetButtonPressDown(button);
        }

        public bool GetButtonPressUp(ControllerButton button)
        {
            return ControllerAdapter.GetButtonPressUp(button);
        }

        public bool GetButtonTouch(ControllerButton button)
        {
            return ControllerAdapter.GetButtonTouch(button);
        }

        public bool GetButtonTouchDown(ControllerButton button)
        {
            return ControllerAdapter.GetButtonTouchDown(button);
        }

        public bool GetButtonTouchUp(ControllerButton button)
        {
            return ControllerAdapter.GetButtonTouchUp(button);
        }

        public void TriggerHapticPulse(float hapticStrength)
        {
            ControllerAdapter.TriggerHapticPulse(hapticStrength);
        }

        public Vector2 GetTouchpadAxis()
        {
            return ControllerAdapter.GetTouchpadAxis();
        }
