...
namespace Tobii.XR
{
    ...
    {
        public static Vector3 GetUnityPositionFrom(TobiiVector3 pos, Vector3 headToCenterEyeTranslation)
        {
            // Convert from Tobii to Unity coordinate system
            var p = new Vector3(pos.x * -1f / 1000f, pos.y / 1000f, pos.z / 1000f);
            
            // Tobii coordinate system origin is Head, but in Unity we use camera (CenterEye) as origin
            p += headToCenterEyeTranslation;
            return p;
        }
        ...
        private static void FillGazeRayFrom(ref TobiiXR_GazeRay gazeRay, tobii_validity_t originValidity, TobiiVector3 origin, tobii_validity_t directionValidity, TobiiVector3 direction, Vector3 headToCenterEyeTranslation)
        {
            ...
            gazeRay.Origin = GetUnityPositionFrom(origin, headToCenterEyeTranslation);
