...
namespace Tobii.XR
{
    ...
    public class StreamEngineTracker
    {
        ...
        public bool ConvergenceDistanceSupported;
        public bool PositionGuideSupported;

#if UNITY_ANDROID
        private static AndroidJavaObject _androidBinderHelper;
        private static bool _binderInstalled;
        
        [RuntimeInitializeOnLoadMethodAttribute(RuntimeInitializeLoadType.AfterAssembliesLoaded)]
        private static void InitializeAndroidBinder()
        {
            var unityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer");
            var currentActivity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity");
            var applicationContext = currentActivity.Call<AndroidJavaObject>("getApplicationContext");
            var factory = new AndroidJavaClass("com.tobii.binder.TobiiBinderFactory");
            _androidBinderHelper = factory.CallStatic<AndroidJavaObject>("create");
            Debug.Log("Initializing Tobii binder...");
            _binderInstalled = _androidBinderHelper.Call<bool>("start", applicationContext, null);
            Debug.Log("Tobii binder response " + _binderInstalled);
        }
#endif
        ...
        public StreamEngineTracker(StreamEngineTracker_Description description)
        {
            ...
            if (_binderInstalled)
            {
                // Block until runtime is ready (up to 20*50 ms)
                for (var i = 0; i < 20; i++)
                {
                    if (_androidBinderHelper.Call<bool>("isReady")) break;
                    Debug.Log("Tobii Runtime is not ready. Sleeping 50 ms");
                    Thread.Sleep(50);
                }
            }
#endif
        ...
        public void Start(StreamEngineTrackerStartInfo startInfo)
        {
            ...
            if (startInfo.WearableAdvancedDataCallback != null)
            {
                ...
                if (result != tobii_error_t.TOBII_ERROR_NO_ERROR)
                {
                    throw new Exception("Failed to subscribe to advanced eye tracking data: " + result);
                }
            }
            
            if (startInfo.WearableFoveatedDataCallback != null)
            {
                Interop.tobii_stream_supported(Context.Device, tobii_stream_t.TOBII_STREAM_WEARABLE_FOVEATED_GAZE, out var foveatedGazeSupported);
                if (foveatedGazeSupported)
                {
                    ...
                    if (result != tobii_error_t.TOBII_ERROR_NO_ERROR)
                    {
                        throw new Exception("Failed to subscribe to foveated eye tracking data: " + result);
                    }    
                }
                else
                {
                    Debug.Log("Tobii Spotlight not supported on this device");
                }
            }
        }
        ...
        private void CheckForCapabilities(IntPtr context)
        {
            Interop.tobii_capability_supported(context,
                tobii_capability_t.TOBII_CAPABILITY_COMPOUND_STREAM_WEARABLE_CONVERGENCE_DISTANCE, out ConvergenceDistanceSupported);

            Interop.tobii_capability_supported(context,
                tobii_capability_t.TOBII_CAPABILITY_COMPOUND_STREAM_WEARABLE_USER_POSITION_GUIDE_XY, out PositionGuideSupported);
        }
